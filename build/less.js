...
[
  {
    src: "src/less/uikit.less",
    dist: `dist/css/uikit-core${rtl ? "-rtl" : ""}.css`,
  },
  {
    src: "src/less/uikit.theme.less",
    dist: `dist/css/uikit${rtl ? "-rtl" : ""}.css`,
  },
].forEach((config) => compile(config.src, config.dist));

const themes = fs.existsSync("themes.json")
  ? JSON.parse(fs.readFileSync("themes.json"))
  : {};
...
glob.sync("custom/*.less").forEach((file) => {
  ...
  compile(file, dist).catch(({ message }) => {
  ...
  if (rtl) {
    output = postcss([
      (css) => {
        css.insertBefore(
          css.nodes[0],
          postcss.comment({ text: "rtl:begin:rename" })
        );
        css.insertAfter(
          css.nodes[css.nodes.length - 1],
          postcss.comment({ text: "rtl:end:rename" })
        );
      },
      rtlcss({
        stringMap: [
